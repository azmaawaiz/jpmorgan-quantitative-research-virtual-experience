#This script contains the code of Task 1 and Task 2.
...
# -------------------------------------
# CONTRACT PRICING ENGINE & ESTIMATOR
# -------------------------------------

print("\n")
print("=" * 70)
print("NATURAL GAS CONTRACT VALUE CALCULATOR")
print("=" * 70)

print(f"Available date bounds: {price_curve['Date'].min().date()} -> {price_curve['Date'].max().date()}")

while True:
    print("\n" + "-"*50)
    print("Enter contract details or type 'q' at any prompt to exit.")
    
    inj_input = input("Enter Injection Date (YYYY-MM-DD): ").strip()
    if inj_input.lower() == 'q': break
        
    with_input = input("Enter Withdrawal Date (YYYY-MM-DD): ").strip()
    if with_input.lower() == 'q': break
        
    vol_input = input("Enter Volume of gas (in MMBtu): ").strip()
    if vol_input.lower() == 'q': break
    inj_rate_input = input("Enter Injection Rate (MMBtu/day): ").strip()
    if inj_rate_input.lower() == 'q': break

    max_storage_input = input("Enter Maximum Storage Capacity (MMBtu): ").strip()
    if max_storage_input.lower() == 'q': break

    fee_rate_input = input("Enter Monthly Storage Fee rate: £").strip()
    if fee_rate_input.lower() == 'q': break

    try:
        # Convert text inputs to variables
        inj_date = pd.to_datetime(inj_input)
        with_date = pd.to_datetime(with_input)
        volume = float(vol_input)  
        injection_rate = float(inj_rate_input)# The withdrawal rate is assumed to be equal to the injection rate.
        max_storage = float(max_storage_input)
        monthly_fee_rate = float(fee_rate_input)
        if volume > max_storage:
           print("\nError: Requested volume exceeds the maximum storage capacity.")
           continue
        
        if with_date <= inj_date:
            print("\nError: Withdrawal date must be after the injection date.")
            continue

        # Get prices from estimation curve
        inj_price = estimate_price(inj_date)
        with_price = estimate_price(with_date)
        
        # Calculate timeline duration metrics
        # (approximate months calculation based on day difference / 30.44 days per month)
        day_diff = (with_date - inj_date).days
        months = day_diff / 30.44
        # Maximum amount that can be injected within the available time
        maximum_possible_injection = injection_rate * day_diff * 2 # Assuming injection and withdrawal rates are equal, hence multiplied by 2
        if volume > maximum_possible_injection:
           print("\nError: The requested volume cannot be injected within the available time.")
           continue
        # Financial Mathematics Pipeline

        overall_storage_fees = monthly_fee_rate * months
        
        profit_without_fees = (with_price - inj_price) * volume
        overall_profit_without_fees = profit_without_fees
        
        expected_profit = overall_profit_without_fees - overall_storage_fees

        # Output breakdown metrics
        print("\n" + "=" * 50)
        print("CONTRACT EXECUTION SUMMARY")
        print("=" * 50)
        print(f"Injection Price   : £{inj_price:.2f} / MMBtu")
        print(f"Withdrawal Price  : £{with_price:.2f} / MMBtu")
        print(f"Estimated Months  : {months:.2f}")
        print(f"Injection Rate    : {injection_rate:,.2f} MMBtu/day")
        print(f"Maximum Storage   : {max_storage:,.2f} MMBtu")
        print(f"Profit Without Fees (per MMBtu): £{profit_without_fees:.2f}")
        print("-" * 50)
        print(f"Overall Storage Fees         : £{overall_storage_fees:.2f}")
        print(f"Overall Profit Without Fees  : £{overall_profit_without_fees:.2f}")
        print(f"Expected Profit              : £{expected_profit:.2f}")
        print("=" * 50)

    except Exception as e:
        print(f"\nCalculation Error: {e}. Please check your dates/inputs and try again.")
